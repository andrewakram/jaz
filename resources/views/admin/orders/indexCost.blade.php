                ...
                <div class="row">
                    <div class="col">
                        <div class=" {{Session::get("lang") =="ar" ? 'page-header-right' : 'page-header-left'}}">
                            <h3>{{trans('admin.finishedOrders')}} </h3>
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="{{route('dashboard')}}"><i data-feather="home"></i></a></li>
                                <li class="breadcrumb-item active">{{ trans('admin.finishedOrders')}}</li>
                            </ol>
                        </div>
                        <br>
                        ...
                        <a href="costs/export" class="btn btn-warning" style="float:{{Session::get("lang") =="ar" ? 'left' : 'right'}}"> {{ trans('admin.exportJAZmoney')}}<i class="fa fa-file-excel-o"></i></a>

                        {{--@include('admin.layouts.search')--}}

                        <div class="card-body" dir="{{Session::get("lang") =="ar" ? 'rtl' : 'ltr'}}">
                            <form class="form-horizontal" action="{{asset(Session::get("lang").'/admin/searchCost')}}" method="get">
                                <div class="form-row">
                                    <div class="input-group">
                                        <input type="text" class="form-control" name="search2" value="{{isset($search2) ? $search2 : ''}}" placeholder="{{ trans('admin.searchWorker')}}" />
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-12 col-lg-12 col-xl-12">
                    <h4># {{ trans('admin.finishedOrders')}} : {{sizeof($orders)}}</h4>
                                    ...
                                    @if(count($orders) > 0 )
                                        @foreach($orders as $order)
                                            <tr>
                                                ...
                                                <td>{{isset($order->worker->name)? $order->worker->name: ''}}</td>

...
class CostsUserExport implements FromView
{
    /**
    ...
    */
    ...
    public function view(): View
    {
        ...
        $orders = $get_orders->search3(Session::get("arr_search3"));
        //////////
        ...
        global $userTotalOrders;
        global $jazTotalMoney;
        global $govMoney;

        foreach ($orders as $order)
        {
            $userTotalOrders += $order->order_total;
            ...
            $provider = Worker::where('id',$order->worker_id)->select('provider_id')->first()->provider_id;
            if($provider == 1){
                ...
                if($order->paid_status==0){
                    $order['worker_money'] = ($order->finish_price * (100-$fee) ) / 100;
                    $order['jaz_money'] = ($order->finish_price * $fee) / 100;
                    $jazTotalMoney += ($order->finish_price * $fee) / 100;
                }elseif($order->paid_status==1){
                    $order['worker_money'] = ($order->finish_price * (100-$fee) ) / 100;
                    $order['jaz_money'] = ($order->finish_price * $fee) / 100;
                }
            ...
            $govfee = Admin::whereId(1)->select('id','gov_fee')
                ->first()->gov_fee;
            $order['gov_money'] = ($order->finish_price * $govfee) / 100;
            $govMoney += ($order->finish_price * $govfee) / 100;
            //
        ...
        return view('admin.exports.costsUser', [
            'orders' => $orders,

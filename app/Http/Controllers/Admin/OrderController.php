...
class OrderController extends Controller
{
    ...
    protected function indexUserCost()
    {
        $orders = $this->orderRepository->indexUserCost();
        $cats = Category::where('type',1)
            ->where('parent_id',Null)->get();
        return view('admin.orders.indexUserCost',compact('orders','cats'));
    }
    ...
    {
        Session::put("search2",$request->search2);
        Session::put("from",$request->from);
        Session::put("to",$request->to);
    ...
    {
        Session::put("search3",$request->search3);
        Session::put("from",$request->from);
        Session::put("to",$request->to);
        $orders = $this->orderRepository->search3($request);
        return view('admin.orders.indexUserCost',compact('orders'));
    }
    ...
    public function costsUser_export(Request $request)
    {
        ...
        Session::put("arr_search3",$arr_search3);



        return Excel::download(new CostsUserExport, 'jaz_costs_users_invoice.xlsx');

...
class OrdersRepository implements OrdersRepositoryInterface
{
    ...
    public function create($input)
    {
        $array = array(
            ...
            'date' => $input->date,
            'time' => $input->time,
            'order_choice' => $input->order_choice,
        );
        ...
        if($order)
        {
            ...
            if(isset($input->hours)){
                ...
                $isPeriod=Category::whereId($input->cat_id)
                    ->select('has_period')->first()->has_period;

                if($isPeriod == true){
                    $price=Category::whereId($input->cat_id)
                        ->select('price')->first()->price;

                    $order->update(['order_total' => $price * $input->hours]);
    ...
    public function third_cat_order($order_id, $cat_id, $hours)
    {
        ...
        $order = Order::whereId($order_id)->select('id','user_id','cat_id','order_choice')->first();
        //
        $cats = Category::where("id",$order->cat_id)->first();
        //
        $filter_order = Order::filterbylatlng2($order->user->lat,$order->user->lng,50,'workers',$order->cat_id,$order->order_choice);
    ...
    public function activeRequests($user_id)
    {
        $orders = Order::whereUserId($user_id)->where('order_action',0)->where('worker_id',null)
            ->select('id','created_at')->where('cancel_reason',NULL)->latest()->get();
        $active_orders = [];
